...
export default function PaymentPageClient() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const data = await res.json(); if (!res.ok) throw new Error(data.error||'Fee calculation error');
        setDeliveryFee(data.fee);
      } catch { toast.error("Sorry, we can't deliver to that address yet."); setDeliveryFee(null); }
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      const orderRes = await fetch('/api/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cartItems,fulfillmentMethod,scheduledTime,deliveryFee,customerInfo:{name:customerName,email:customerEmail,phone:customerPhone,address:deliveryAddress}})});
      const {orderId,error:orderError} = await orderRes.json(); if(orderError) throw new Error(orderError);
      const linkRes = await fetch('/api/create-payment-link',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cartItems,fulfillmentMethod,scheduledTime,customerInfo:{name:customerName,email:customerEmail,phone:customerPhone,address:deliveryAddress},orderId})});
  ...
  const handleDeliveryAddressChange = (val:string) => { setDeliveryAddress(val); };
